  ...
  for (var y = 0; y < this.height; y++) {
    ...
    for (var x = 0; x < this.width; x++) {
      ...
      // Because there is a third case (space ' '), use an "else if" instead of "else"
        ...
        fieldType = "floater";
      // "Push" the field Type, which is a string, onto the gridLine array (at the end).
      gridLine.push(fieldType);
    }
    // Push the entire row onto the array of rows.
    this.grid.push(gridLine);
  }
  this.player = this.actors.filter(function(actors) {
...
SlowDown.prototype.act = function(step) {
...
Level.prototype.obstacleAt = function (pos, size) {
  ...
  //cosider sides, top and bottom of level as walls
...
Player.prototype.moveX = function(step, level, keys) {
  ...
  //Find if there is a obstacle at the newPos
